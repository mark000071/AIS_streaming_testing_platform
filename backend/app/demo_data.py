...
_DEMO_VESSELS = [
    {"mmsi": 230123456, "source": "digitraffic", "ship_class": "cargo",
     "lat0": 60.080, "lon0": 24.700, "heading_deg": 95.0, "sog_kn": 14.5},
    {"mmsi": 230987654, "source": "digitraffic", "ship_class": "tanker",
     "lat0": 60.020, "lon0": 25.300, "heading_deg": 210.0, "sog_kn": 11.0},
    {"mmsi": 257345678, "source": "kystdatahuset", "ship_class": "passenger",
     "lat0": 59.500, "lon0": 10.600, "heading_deg": 340.0, "sog_kn": 18.0},
    {"mmsi": 257765432, "source": "kystdatahuset", "ship_class": "fishing",
     "lat0": 59.200, "lon0": 5.050, "heading_deg": 55.0, "sog_kn": 7.5},
    {"mmsi": 230555111, "source": "digitraffic", "ship_class": "cargo",
     "lat0": 59.950, "lon0": 25.600, "heading_deg": 250.0, "sog_kn": 12.0},
]
...
def _mcmnet_fan(cv_xy, gt_xy, rng):
    ...
    for k in range(N_HYPOTHESES):
        ...
        hyps.append([(cx + pull * (gx - cx) + rng.gauss(0, 15.0),
                      cy + pull * (gy - cy) + rng.gauss(0, 15.0))
                     for (cx, cy), (gx, gy) in zip(cv_xy, gt_xy)])
    return hyps
...
def _blend(a_xy, b_xy, alpha):
    ...
    candidate, b = CV."""
    return [(alpha * ax + (1 - alpha) * bx, alpha * ay + (1 - alpha) * by)
            for (ax, ay), (bx, by) in zip(a_xy, b_xy)]
...
def generate(seed: int = 20260101, n_ticks: int = 6) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for vessel in _DEMO_VESSELS:
        ...
        for tick in range(n_ticks):
            ...
            def block(xy):
                return _latlon_block(xy, anchor_lat, anchor_lon)
            ...
            predictions.append({
                ...
                "mcmnet": {
                    ...
                    "top1_index": 0,
                    "scorer": dict(block(scored), index=0, alpha=SCORER_ALPHA,
                                   artifact="demo-scorer"),
                    "routed": dict(block(routed), mode="maneuver" if maneuver else "straight",
                                   source="scorer" if maneuver else "kalman",
                                   router="demo-router"),
                    "routed_cvkal": dict(block(routed_cvkal),
            ...
            scores.append({
                ...
                "routed_cvkal_ade": cvkal_ade, "routed_cvkal_fde": cvkal_fde,
